...
GPU_NUM=0
SAVE_PATH="save_path/caspl_cocoop_base_to_novel"
...
CFG_S2_LIST = [["vit_b16_c8_ep10_batch1"   ,8]]
...
for (DATASET, DATA) in dataset_list:
    ...
    for (CFG_S1, KD_N_CTX_VISION_TEACHER, KD_PROMPT_DEPTH_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT) in CFG_S2_LIST:
            OUT_PUT = ("caspl_cocoop_s1_v" + str(KD_N_CTX_VISION_TEACHER) + "_t"
                       + str(KD_N_CTX_VISION_TEACHER) + "_s2_v" + str(N_CTX_VISION_STUDENT) +"_t" + str(N_CTX_VISION_STUDENT))
            ...
            for COUNTER in range(1, 4):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/cocoop/base2new_train_cocoop_student.sh \
                      ...
                      {SAVE_PATH} {OUT_PUT_TEACHER} {SAVE_PATH_TEACHER}"
                os.system(cmd)
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/cocoop/base2new_test_cocoop_student.sh \
