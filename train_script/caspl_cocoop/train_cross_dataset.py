...
GPU_NUM=0
SAVE_PATH="save_path/caspl_cocoop_cross_dataset"
PATH_TEACHER="save_path/caspl_teacher"

CFG_S1_DICT = [["vit_b16_c2_ep20_batch128_lr0025_4+4ctx_l3", 4, 3]]
CFG_S2_LIST = [["vit_b16_c4_ep10_batch1_ctxv1"   ,4, 1]]
...
for (DATASET, DATA) in dataset_list:
    ...
    for (CFG_S1, KD_N_CTX_VISION_TEACHER, PROMPT_DEPTH_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT, LAYER_STUDENT) in CFG_S2_LIST:
            OUT_PUT = ("caspl_cocoop_s1_v" + str(KD_N_CTX_VISION_TEACHER) +
                       "_t" + str(KD_N_CTX_VISION_TEACHER) + "_s2_int_v" + str(N_CTX_VISION_STUDENT) \
            ...
            for COUNTER in range(1, 4):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/cocoop/xd_train_student.sh \
                ...
                for (DATASET_TEST, DATA_TEST) in dataset_list_test:
                    cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/cocoop/xd_test_student.sh \
