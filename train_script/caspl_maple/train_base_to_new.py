...
GPU_NUM=0
SAVE_PATH="save_path/caspl_maple_base_to_novel"
...
for (DATASET, DATA) in dataset_list:
    ...
    for (CFG_S1, KD_N_CTX_VISION_TEACHER, KD_PROMPT_DEPTH_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT) in CFG_S2_LIST:
            OUT_PUT = "caspl_maple_s1_v" + str(KD_N_CTX_VISION_TEACHER) + "_t" + str(KD_N_CTX_VISION_TEACHER) + "_s2_v" + str(N_CTX_VISION_STUDENT) +"_t" + str(N_CTX_VISION_STUDENT)
            ...
            for COUNTER in range(1, 4):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/maple/base2new_train_maple_student.sh \
                ...
                os.system(cmd)
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/maple/base2new_test_maple_student.sh \
