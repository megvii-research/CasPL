...
GPU_NUM=0
SAVE_PATH="save_path/caspl_promptsrc_cross_dataset"
PATH_TEACHER = "save_path/caspl_teacher"

CFG_S1_DICT = [["vit_b16_c2_ep20_batch128_lr0025_4+4ctx_l1", 4, 1]]
CFG_S2_LIST = [["vit_b16_c2_ep20_batch4_8+8ctx_cross_datasets_l3"   ,8]]
#kd_num_depth_12_imagenet_only
...
for (DATASET, DATA) in dataset_list:
    ...
    for (CFG_S1, KD_N_CTX_VISION_TEACHER, PROMPT_DEPTH_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT) in CFG_S2_LIST:
            OUT_PUT = ("caspl_promptsrc_s1_v" + str(KD_N_CTX_VISION_TEACHER) +
                       "_t" + str(KD_N_CTX_VISION_TEACHER) + "_s2_v" + str(N_CTX_VISION_STUDENT) +
                       "_t" + str(N_CTX_VISION_STUDENT) + "_" + str(CFG_S1))
            ...
            for COUNTER in range(1, 2):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/promptsrc/xd_train_student.sh \
                ...
                for (DATASET_TEST, DATA_TEST) in dataset_list_test:
                    cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/promptsrc/xd_test_student.sh \
