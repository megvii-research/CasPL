...
CFG_S2_LIST = [["vit_b16_c2_ep20_batch4_8+8ctx_l12"   ,8]]


for (DATASET, DATA) in dataset_list:
    print(DATASET, DATA)
    for (CFG_S1, KD_N_CTX_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT) in CFG_S2_LIST:
            OUT_PUT = "caspl_promptsrc_s1_v" + str(KD_N_CTX_VISION_TEACHER) + "_t" + str(KD_N_CTX_VISION_TEACHER) + "_s2_v" + str(N_CTX_VISION_STUDENT) +"_t" + str(N_CTX_VISION_STUDENT)
            ...
            for COUNTER in range(1, 4):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/promptsrc/base2new_train_promptsrc_student.sh \
                      {DATA} {DATASET} {COUNTER} {OUT_PUT} {DIR_NAEM_STUDNET} {DIR_NAME_TEACHER} \
                      {RESULT_PATH} {CFG_S2} {CFG_NAME} \
                      {KD_N_CTX_VISION} {KD_N_CTX_TEXT} \
                      {SAVE_PATH} {SAVE_PATH_TEACHER} {OUT_PUT_TEACHER}"
                os.system(cmd)
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/promptsrc/base2new_test_promptsrc_student.sh \
