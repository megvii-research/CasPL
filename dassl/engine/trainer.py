...
class TrainerBase:
    ...
    def load_model_one_stage(self, new_name, directory, epoch=None):
        if not directory:
            print(
                "Note that load_model() is skipped as no pretrained "
                "model is given (ignore this if it's done on purpose)"
        ...
        for name in names:
            model_path = osp.join(directory, new_name, model_file)
            ...
            epoch = checkpoint["epoch"]
            val_result = checkpoint["val_result"]
            print_info("The weight of one_stage is loading:" + new_name)

            teacher_list = ['prompt_learner_teacher.ctx', 'VPT_FIRST', 'VPT_shallow_FIRST']
            model_dict = self._models[name].state_dict()
            state_dict_teacher = {k: v for k, v in state_dict.items() if k in model_dict and any(keyword in k for keyword in teacher_list)}
            enabled = set()
            for n, p in state_dict_teacher.items():
                enabled.add(n)
            enabled = sorted(enabled)
            print_info("The second stage loads the weights of the first stage:")
            print_info(enabled)

            self._models[name].load_state_dict(state_dict_teacher, strict=False)
...
class SimpleTrainer(TrainerBase):
    ...
    def after_epoch(self):
        ...
        if meet_checkpoint_freq or last_epoch or (self.epoch+1) % 10==0:
