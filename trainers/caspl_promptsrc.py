...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if cfg.KD.FIRST_TRAIN == False and cfg.KD.SECOND_TRAIN != False:
            n_ctx_student = n_ctx
            n_ctx_teacher = cfg.KD.N_CTX_TEXT
            ...
            if ctx_init and n_ctx_student <= 4:
                ...
                ctx_init = ctx_init_student + " " +ctx_init_teacher# a photo of a X X X X
                print_info("PormptSRC is using the second init and the ctx_init should be: a photo of a X*n_ctx_teacher")
                prompt = clip.tokenize(ctx_init)
                with torch.no_grad():
                    embedding = clip_model.token_embedding(prompt).type(dtype)#[1, 77, 512]
                ctx_vectors = embedding[0, 1: 1 + n_ctx_student, :]#[4, 512]
                prompt_prefix = ctx_init
            else:
                print_info("PormptSRC is using the second init and the ctx_init should be: X*(n_ctx_teacher+n_ctx_student)")
                ctx_vectors = torch.empty(n_ctx_student , ctx_dim, dtype=dtype)
                nn.init.normal_(ctx_vectors, std=0.02)
                prompt_prefix = " ".join(["X"] * (n_ctx))


        elif ctx_init and n_ctx <= 4:
        ...
        self.fixed_embeddings = torch.cat(all_teacher_features, dim=1).mean(dim=1)#[50 ,512]
    ...
    def construct_prompts(self, ctx, prefix, suffix, label=None, ctx_teacher=None):
        ...
        return prompts#[50, 77, 512]

    def forward(self, ctx_teacher=None):
        ctx = self.ctx#[4, 512]
        if ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1)#[50, 4, 512]
        if ctx_teacher != None and ctx_teacher.dim()==2:#[4, 512]
            ctx_teacher = ctx_teacher.unsqueeze(0).expand(self.n_cls, -1, -1)#[50, 4, 512]

        prefix = self.token_prefix
        suffix = self.token_suffix
        prompts = self.construct_prompts(ctx, prefix, suffix, ctx_teacher=ctx_teacher)
...
class VLPromptLearner_teacher(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        print(f'Initial text context: "{prompt_prefix}"')
        print(f"Number of context words (tokens) for Language prompting: {n_ctx}")
        print(f"Number of context words (tokens) for Vision prompting: {cfg.KD.N_CTX_VISION}")
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        prompts = self.prompt_learner(self.prompt_learner_teacher.ctx)
        ...
        if self.prompt_learner.training:
            ...
            return F.cross_entropy(logits,label), \
                   text_features, fixed_embeddings, \
                   zero_shot_features, image_features, \
...
class CasPL_PromptSRC(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.KD.SECOND_PARAMETER_USE != False:
            print_info("KD.SECOND_PARAMETER_USE is True, means you are updating the second parameters")
            for name, param in self.model.named_parameters():
                if name_to_update not in name:
                    # Make sure that VPT prompts are updated
                    if any(keyword in name for keyword in ["VPT", "teacher", "FIRST"]):
                        param.requires_grad_(True)
                    else:
                        param.requires_grad_(False)
                else:
                    if any(keyword in name for keyword in ["ZS_image_encoder"]):
                        param.requires_grad_(False)
        else:
            for name, param in self.model.named_parameters():
                if name_to_update not in name:
                    ...
                    if any(keyword in name for keyword in ["ZS_image_encoder", "teacher", "VPT_shallow_FIRST"]):
                        param.requires_grad_(False)
