...
class ResidualAttentionBlock_IVLP(nn.Module):
        ...
        if i != 0:
            ...
            if self.add_prompt:
                if self.text_layer:
                    ...
                    self.n_ctx_text_second = design_details["language_ctx"] + self.cfg.KD.N_CTX_TEXT # hyperparameter
    ...
    def forward(self, x: torch.Tensor):
        ...
        # cfg.KD.USE_IVLP is used for controlling the CoOp and CoCoOp.
        # cfg.KD.USE_IVLP==False CoOp and CoCoOp in the first stage.
        # cfg.KD.USE_IVLP==True CoOp and CoCoOp in the second stage.


        if self.add_prompt:
            # Also see if this is textual transformer layer or not
            if not self.text_layer:
                # PromptSRC, CoOp and CoCoOp in the first stage or CoOp and CoCoOp in the second stage.
                ...
                # PromptSRC, CoOp and CoCoOp in the first stage or CoOp and CoCoOp in the second stage.
                if (self.cfg.KD.FIRST_TRAIN != False and self.cfg.KD.SECOND_TRAIN == False) or self.cfg.KD.USE_IVLP:
                    if self.cfg.KD.USE_IVLP:#CoOp and CoCoOp in the second stage
                        prefix = x[:1 + self.n_ctx_text, :, :]
                        suffix = x[1 + self.n_ctx_text_second:, :, :]
                    else:#PromptSRC, CoOp and CoCoOp in the first stage
